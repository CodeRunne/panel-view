...
app.post("/account/email", (req, res) => {
    const { currentEmail, updatedEmail } = req.body;
    const { users } = database;
    console.log(currentEmail, updatedEmail)

    let existingUser = users.find(user => user.email === currentEmail);

    if(!existingUser) {
        const updateUserPassword = users.map(user => user.email === currentEmail ? {...user, email: updatedEmail} : user);
        console.log(updateUserPassword);
